...
def stylize(network, initial, initial_noiseblend, content, styles, preserve_colors, iterations,
    ...
    g = tf.Graph()
    with g.as_default(), g.device('/gpu:0'), tf.compat.v1.Session() as sess:
    ...
    g = tf.Graph()
    with g.as_default(), g.device('/gpu:0'), tf.compat.v1.Session() as sess:
        image = tf.compat.v1.placeholder('float', shape=style_shapes)
        net = vgg.net_preloaded(vgg_weights, image, pooling)
    ...
    with tf.Graph().as_default():
        ...
        content_layers_weights = content_layers_weights
        ...
        style_loss = 0
        style_losses = []
        print('----------------laoding style layers -------------------------')
        for style_layer in STYLE_LAYERS:
            layer = net[style_layer]
            _, height, width, number = map(lambda i: i, layer.get_shape())
